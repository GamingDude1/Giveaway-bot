  ...
  run: async (client, message, args) => {
    ...
    if (message.guild) {
      ...
      let embed = new MessageEmbed()
        ...
        .setTitle('Help')
        .setURL('https://discord.gg/9bxBmbaV6Z')
        .setColor('7289da')
        .setDescription(`Get helps :D`)
    ...
    if (!message.guild) {
      let embed = new MessageEmbed()
        .setAuthor(client.user.username, client.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }), 'https://discord.gg/9bxBmbaV6Z')
        .setThumbnail(client.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }))
        .setTitle('Help')
        .setURL('https://discord.gg/9bxBmbaV6Z')
        .setColor('7289da')
        ...
        .addField(`🔍 ${config["Bot_Info"].prefix}reroll [message-id]`, 'Message-ID has to be the **ID** of the giveaway message.\n**Not the link!**')
        .addField('👥 Example:', `⌨️ ${config["Bot_Info"].prefix}reroll 892678258946659587\n➡️ Selects new winners for the giveaway with the message-ID \`892678258946659587\`.`)
        .setFooter('Made with 💖 and discord.js by Hasibix', client.user.displayAvatarURL({ format: 'png', dynamic: true, size: 1024 }))
      message.author.send(embed);
    }
  },
};
